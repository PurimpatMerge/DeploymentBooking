...
export const findPrice = async (req, res, next) => {
  try {
    ...
    if (!dates) {
      ...
      let totalPrice = priceGeter.reduce(
        (sum, { price }) => sum + parseInt(price, 10),
        0
      );
...
export const findPriceNormal = async (req, res, next) => {
  try {
    const { userStartDate, userEndDate, startPrice } = req.params;
    let priceGeter = [];
